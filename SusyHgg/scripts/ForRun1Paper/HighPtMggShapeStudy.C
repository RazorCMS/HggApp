...
void PlotMggShapeVsRsq(bool useMC = false, string category = "highres") {
 ...
 MGG_MR400ToInf_Rsq0p00To0p015->SetMaximum(0.15);
...
void PlotMggShapeVsMR(bool useMC = false, string category = "highres") {
  ...
  if (!useMC) {
    ...
    //fileData = new TFile("/afs/cern.ch/user/s/sixie/eos/cms/store/group/phys_susy/razor/run1/HggRazor/AlexRaidDisk/DiPhotonJetsBox_M60_8TeV-sherpa__Summer12_DR53X-PU_S10_START53_V7C-v1_v9__WEIGHTED.root","READ");
    fileData = new TFile("/afs/cern.ch/user/s/sixie/eos/cms/store/group/phys_susy/razor/run1/HggRazor/AlexRaidDisk/GJet_Pt40_doubleEMEnriched_TuneZ2star_8TeV-pythia6_v9.root","READ");
  }
 
  TTree *tree = (TTree*)fileData->Get("SusyHggTree");
  
  TH1D *MGG_MR150ToInf_Rsq0p00ToInf = new TH1D("MGG_MR150ToInf_Rsq0p00ToInf",";m_{#gamma#gamma} (GeV); Fraction ", 20,100,400);
  TH1D *MGG_MR300To400_Rsq0p05ToInf = new TH1D("MGG_MR300To400_Rsq0p05ToInf",";m_{#gamma#gamma} (GeV); Fraction ", 20,100,400);
  TH1D *MGG_MR400ToInf_Rsq0p05ToInf = new TH1D("MGG_MR400ToInf_Rsq0p05ToInf",";m_{#gamma#gamma} (GeV); Fraction ", 20,100,400);

  string categoryCutString = "";
  if (category == "highpt") categoryCutString = " && ptgg >= 80 ";
  else if (category == "highres") categoryCutString = " && ptgg < 110 && pho1_sigEoE < 0.015 && pho2_sigEoE < 0.015 ";
  else if (category == "lowres") categoryCutString = " && ptgg < 110 && !(pho1_sigEoE < 0.015 && pho2_sigEoE < 0.015) ";

  string controlRegionCutString = "";
  //if (useMC) controlRegionCutString = "&& pho1_pass_iso && pho2_pass_iso";
  if (useMC) controlRegionCutString = "";
  else controlRegionCutString = "&& (!pho1_pass_iso || !pho2_pass_iso) ";
  // else controlRegionCutString = "&& pho1_pass_iso && pho2_pass_iso ";
  //else controlRegionCutString = "&& pho1_pass_iso && pho2_pass_iso";
 ...
 cv = new TCanvas("cv","cv", 800,600); 
 MGG_MR150ToInf_Rsq0p00ToInf->SetStats(0);
 MGG_MR300To400_Rsq0p05ToInf->SetStats(0);
 MGG_MR400ToInf_Rsq0p05ToInf->SetStats(0);
 MGG_MR150ToInf_Rsq0p00ToInf->Draw("hist");
 MGG_MR150ToInf_Rsq0p00ToInf->SetMaximum(0.25);
 MGG_MR150ToInf_Rsq0p00ToInf->SetMinimum(0.0);
 MGG_MR150ToInf_Rsq0p00ToInf->GetXaxis()->SetTitleSize(0.045);
 MGG_MR150ToInf_Rsq0p00ToInf->GetYaxis()->SetTitleSize(0.045);
 MGG_MR150ToInf_Rsq0p00ToInf->GetYaxis()->SetTitleOffset(0.8);
 
 MGG_MR150ToInf_Rsq0p00ToInf->SetLineColor(kMagenta); 
 MGG_MR150ToInf_Rsq0p00ToInf->SetLineWidth(4); 
 MGG_MR150ToInf_Rsq0p00ToInf->SetMarkerStyle(8); 
 MGG_MR150ToInf_Rsq0p00ToInf->SetMarkerSize(1.3); 
 MGG_MR150ToInf_Rsq0p00ToInf->SetMarkerColor(kBlack); 
